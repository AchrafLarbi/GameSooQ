...
const ConsoleDetailsOverlay = ({ selectedConsolePost, onClose }) => {
  const dispatch = useDispatch();
  const [username, setUsername] = useState("Loading...");
  const [currentImageIndex, setCurrentImageIndex] = useState(0);

  // Destructure the game properties from selectedConsole
  const {
    condition,
    created_at,
    delivery,
  ...
  return (
    // Overlay wrapper - centered on screen with semi-transparent background and scrollable
    <div className="fixed inset-0 bg-black bg-opacity-75 z-50 overflow-y-auto">
      <div className="min-h-screen flex items-center justify-center p-4">
        {/* Overlay container */}
        <div className="flex flex-col max-w-2xl w-full mx-auto bg-black text-white overflow-hidden rounded-3xl">
              ...
              style={{
                backgroundImage:
                  images && images.length > 0
                    ? `url(${images[currentImageIndex || 0]})`
                    : "none",
              }}
            >
              ...
              {images && images.length > 1 && (
                <>
                  <button
                    className="absolute left-2 top-1/2 -translate-y-1/2 bg-gray-200 bg-opacity-50 rounded-full p-2 text-gray-800 z-10 hover:bg-opacity-70"
                    onClick={() =>
                  ...
                  <button
                    className="absolute right-2 top-1/2 -translate-y-1/2 bg-gray-200 bg-opacity-50 rounded-full p-2 text-gray-800 z-10 hover:bg-opacity-70"
                    onClick={() =>
                      setCurrentImageIndex((prev) =>
...
export default ConsoleDetailsOverlay;
